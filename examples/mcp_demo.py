        ...
        async with ClientSession(read, write) as session:
            ...
            try:
                result = await session.call_tool(
                    "list_historic_hurricanes", {"year": 2017, "limit": 10}
                )
                data = json.loads(result.content[0].text)
                if data.get("success"):
            ...
            data = json.loads(result.content[0].text)
            if data.get("success"):
                hazard_id = data["system_id"]
                print(f"✓ Created hazard system: {hazard_id}")
