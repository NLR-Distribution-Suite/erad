...
@mcp.read_resource()
async def read_resource(uri: AnyUrl) -> str:  # noqa: C901
    ...
    if uri_str.startswith("erad://models/"):
        ...
        if len(path_parts) > 1 and path_parts[1] == "summary":
            ...
            summary_lines = [
                ...
                "",
                f"**Created**: {models[model_name].get('created_at', 'Unknown')}",
                f"**Description**: {models[model_name].get('description', 'No description')}",
                f"**File**: {file_path}",
                "",
                "## Content Summary",
...
@mcp.call_tool()
async def call_tool(name: str, arguments: dict[str, Any]) -> list[TextContent]:  # noqa: C901
    """Handle tool calls."""
    ...
    if name == "list_cached_models":
        ...
        # Load model to get more details
        try:
        ...
        dist_name = arguments.get("distribution_system_name")
        hazard_name = arguments.get("hazard_system_name")
        curve_set = arguments.get("curve_set", "DEFAULT_CURVES")
        ...
        dist_name = arguments.get("distribution_system_name")
        hazard_name = arguments.get("hazard_system_name")
        number_of_samples = arguments.get("number_of_samples", 1)
        ...
        try:
            ...
            result = {
                ...
                "file_size_bytes": file_size,
                "content_keys": list(data.keys()) if isinstance(data, dict) else None,
                "hazard_model_count": len(data.get("models", []))
                if isinstance(data, dict)
                else None,
